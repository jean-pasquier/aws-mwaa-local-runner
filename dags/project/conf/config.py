...
def get_config(key) -> Dict[str, Any]:
    """ Loads the conf files for a given environment.

    The environment folder to load is defined by the `AIRFLOW_ENV` environment variable.
    If not `AIRFLOW_ENV` variable is set, default to the `local` folder.
    ...
    conf_dir = os.path.join(this_dir, 'confs')

    conf_paths = [
        os.path.join(conf_dir, 'base'),
        os.path.join(conf_dir, f'{env}')
    ]

    conf_loader = ConfigLoader(conf_paths)
    conf = conf_loader.get(f'{key}*')
...
class ConfigLoader:
    ...
    ::
        ...
        >>> from pyspark_project.common.config import ConfigLoader
        >>>
